...
public class Main {
	...
	public Main() {
		...
		try {
			...
			UltrasonicSensor ultrasonicSensor = new UltrasonicSensor(sensor.getMode("Distance"));
			...
			while (true) {
				...
				if (WlanConnected && BTConnected) {
					...
					if (!WlanMessage.equals("Nix")) {
						xPosition = Integer.valueOf(WlanMessage);
					}
					
					// gets distance to user by an ultrasonic-sensor
					zPosition = ultrasonicSensor.getDistance();
					
					if (zPosition > 50 && !stop) {
						
						if (xPosition > 10 || xPosition < -10) {
							mB.rotateTo(xPosition / 320 * 90, true);
							mC.rotateTo(-xPosition / 320 * 90, false);
						} else {
							mB.rotateTo(0, true);
							mC.rotateTo(0, false);
						}
